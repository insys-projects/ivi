 ...
 *
 *  Purpose:  This sample program initializes the scope, configures the Real-Time eye
 *  		  analysis feature, and measures the eye height and eye width, and displays 
 *            them in a pop-up dialog.
 *
...

{
    ...
    checkErr( agx2k3k_ConfigureRealTimeEye (agx2k3k, "CHAN1", VI_TRUE));
	checkErr( agx2k3k_BuildRealTimeEye (agx2k3k));
	Delay(1);
	checkErr( agx2k3k_ReadRTEyeMeasurement (agx2k3k, AGX2K3K_VAL_EYE_HEIGHT, &height));
	checkErr( agx2k3k_ReadRTEyeMeasurement (agx2k3k, AGX2K3K_VAL_EYE_WIDTH, &width));
	
	sprintf (msgStr, "Eye Height = %g\nEye Width = %g", height, width);

 ...
 *
 *  Purpose:  This sample program initializes the scope, configures the histogram
 *  		  analysis feature, and measures the sum of all hits, and displays 
...

{
    ...
    checkErr( agx2k3k_ConfigureInitiateContinuous (agx2k3k, VI_FALSE));

	checkErr( agx2k3k_ConfigureHistogram(agx2k3k, AGX2K3K_VAL_HISTOGRAM_WAVEFORM));
	checkErr( agx2k3k_ConfigureHistogramWaveform(agx2k3k, AGX2K3K_VAL_HORIZONTAL, 0, 2e-6, 5, 0, "CHAN1"));
	checkErr( agx2k3k_ReadHistogramMeasurement(agx2k3k, AGX2K3K_VAL_HITS, &totalHits));
	sprintf (msgStr, "Hits = %lf", totalHits);

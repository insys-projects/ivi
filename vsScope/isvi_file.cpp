...
void DoubleToShort(double *in, short *out, SCOPE_PARAM *scope_param)
{
	for (int i = 0; i < (int)scope_param->NumSamples; i++) {
		//double scale = 1L << scope_param->AdcBitRange;
		if(scope_param->SampleSize == 2)
			out[i] = (short) ((in[i]*32768.0) / (scope_param->InpRange/2));
		else if (scope_param->SampleSize == 1)
			out[i] = (short)((in[i] * 128.0) / (scope_param->InpRange/2));
	}
...
void    PrepareFileHeader(SCOPE_PARAM *scope_param, int *pChanNums, double *pGains, double *pFreqOffsets)
{
	...
	sprintf(g_str, "\r\nSAMPLING_RATE_______ %f", scope_param->SampleRate);
	strcat(g_fileHeader, g_str);
